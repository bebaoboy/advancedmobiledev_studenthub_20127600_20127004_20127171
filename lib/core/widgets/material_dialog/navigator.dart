...
class DialogNavigator {
  ...
  /// Goes back one page and call onPop.
  ///
  /// Size and alignment will be changed automatically.
  void pop() {
    var p = pages.value.toList().lastOrNull;
    if (p != null && p.onPop != null) p.onPop!();
    pages.value = pages.value.toList()..removeLast();
  }

  /// Closes the entire dialog.
  ///
  /// Call onPop on the first dialog!`
  Future close() async {
    var p = pages.value.toList().firstOrNull;
    if (p != null && p.onPop != null) p.onPop!();
    pages.value.clear();
...
class FluidDialogPage {
  ...
  /// Function to call when the dialog is popped
