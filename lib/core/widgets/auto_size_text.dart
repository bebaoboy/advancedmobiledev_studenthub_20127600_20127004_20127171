...
class AutoSizeText extends StatefulWidget {
  ...
  final EdgeInsets? matchPadding;

  final BoxDecoration? matchDecoration;
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (context, size) {
      ...
      if (widget.words != null &&
          ...
          widget.data != null) {
        final sortedWords = sortWords(widget.words!.entries.toList());
        final boundWords;
        final words = sortedWords.map((entry) => entry.key).toList();
        switch (widget.binding) {
          case HighlightBinding.first:
            boundWords = firstOccurance(words, widget.data!, widget.matchCase);
          case HighlightBinding.last:
            boundWords = lastOccurance(words, widget.data!, widget.matchCase);
          ...
          .map<List<InlineSpan>>((element) {
            if (element is MatchedElement) {
              ...
              try {
                ...
                if (widget.splitOnLongWord && showWord.contains(" ")) {
                ...
                return splittedWords.map((w) {
                  ...
                  return WidgetSpan(
                    ...
                    child: InkWell(
                      ...
                      child: Container(
                        padding: highlightedWord.padding ?? widget.matchPadding,
                        decoration: highlightedWord.decoration ??
                            widget.matchDecoration,
                        child: AutoSizeText(
                          w,
