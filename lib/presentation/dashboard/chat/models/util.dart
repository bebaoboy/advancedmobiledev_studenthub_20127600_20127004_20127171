...
import 'package:boilerplate/presentation/dashboard/chat/widgets/image_gallery.dart';
...
class UnreadHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        color: Chat.theme.unreadHeaderTheme.color,
        margin: EdgeInsets.only(bottom: 24, top: marginTop ?? 0),
        padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 8),
        child: Text(
          Chat.l10n.unreadMessagesLabel,
          style: Chat.theme.unreadHeaderTheme.textStyle,
  ...
  for (var i = messages.length - 1; i >= 0; i--) {
    ...
    if (showUserNames) {
      ...
      if (isFirstInGroup) {
        ...
        if (message.type == AbstractMessageType.text) {
          showName = true;
        } else {
          shouldShowName = true;
        }
      ...
      if (message.type == AbstractMessageType.text && shouldShowName) {
        showName = true;
        shouldShowName = false;
      }
    }
    ...
    if (!nextMessageInGroup && message.type != AbstractMessageType.system) {
      chatMessages.insert(
        0,
        MessageSpacer(
          height: 12,
    ...
    if (message is AbstractImageMessage) {
      if (kIsWeb) {
